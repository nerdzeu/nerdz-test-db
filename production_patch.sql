...
alter table comments_notify add column to_notify boolean not null default true;
alter table posts_notify add column to_notify boolean not null default true;
alter table groups_comments_notify add column to_notify boolean not null default true;
alter table groups_notify add column to_notify boolean not null default true;

-- trigger json notifications when a new notification row is added (first param is the notification type)
CREATE OR REPLACE FUNCTION trigger_json_notification() RETURNS TRIGGER
LANGUAGE plpgsql
AS $$
DECLARE
keys text[];
